...
xlabel('$x_{1}$', 'Interpreter', 'latex'); ylabel('Count'); set(gca, 'Fontsize', 20)
...
loglike = @(theta, dm, width) loglikelihood(theta, x1sams1, blackbox_model, dm, width, Nsim); % dm is the index for the type of distance metric
...
TEMCMC = cell(4,1); timeTEMCMC = zeros(4,1); 
width = [4e-3, 3.5e-2, 4.5e-2, 7e-3]; % Width parameter vector

for i = 1:4
sprintf('Now running simulation batch = %d', i)
logL = @(theta) loglike(theta, i, width(i));
tic;
TEMCMC{i} = TEMCMCsampler('nsamples', N, 'loglikelihood', logL, 'priorpdf', priorpdf, 'priorrnd', priorrnd);
timeTEMCMC(i) = toc;
end

%% Save the data:
save('NASA_LaRC_Challenge_Part1_v2', 'TEMCMC', 'timeTEMCMC')
